...
Farm.prototype.mkhandle = function (method) {
  return function () {
    ...
    if (this.activeCalls >= this.options.maxConcurrentCalls) {
      const msg = `Too many concurrent calls (${this.activeCalls})`
      const err = new MaxConcurrentCallsError(msg)
      const lastArg = last(args)

      if (isFunction(lastArg)) {
        return process.nextTick(lastArg.bind(null, err))
      }
...
Farm.prototype.setup = function (methods) {
  ...
  if (!this.options.autoStart) return iface
